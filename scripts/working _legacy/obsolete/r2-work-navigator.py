...
- Reports:
  ...
  * duplicate archives
- Dependency-free Tkinter navigator with advanced search.
- Always regenerates one canonical search.index.jsonl.
...
def search_index_record(record: WorkRecord) -> dict[str, Any]:
    """One canonical JSONL row used by both search and the Tkinter navigator."""
    manifests = []
    for manifest in record.item_manifests:
        row = dict(manifest)
        if row.get("valid"):
            row["predicted_page_urls"] = {
                "template": row.get("url_template"),
                "first": row.get("first_page_url"),
                "last": row.get("last_page_url"),
                "count": row.get("pages"),
            }
        manifests.append(row)

    status = "complete"
    unresolved = [m for m in manifests if not m.get("valid")]
    if unresolved:
        status = "unresolved"
    elif not record.has_details_json or not record.has_thumb_webp or record.item_json_count == 0:
        status = "incomplete"
    ...
    row = {
        ...
        "status": status,
        "has_details_json": record.has_details_json,
        "has_thumb_webp": record.has_thumb_webp,
        "item_json_count": record.item_json_count,
        "archive_count": record.archive_count,
        "zip_count": record.zip_count,
        "cbz_count": record.cbz_count,
        "archive_paths": record.archive_paths,
        "details_paths": record.details_paths,
        "item_json_paths": record.item_json_paths,
        "thumb_paths": record.thumb_paths,
        "manifests": manifests,
        "possible_duplicate": record.suspicious_duplicate_group,
    }
    searchable = [
        record.name,
        record.remote_path,
        ...
        *record.details_paths,
        *record.item_json_paths,
        *record.thumb_paths,
    ]
    for manifest in manifests:
        searchable.extend(str(manifest.get(key) or "") for key in (
            "item_path", "base_url", "url_template", "first_page_url", "last_page_url", "error"
        ))
    row["search_text"] = " ".join(searchable).casefold()
    return row


def write_search_index(records: list[WorkRecord], output_dir: Path) -> Path:
    """Atomically replace the single canonical search index."""
    output_dir.mkdir(parents=True, exist_ok=True)
    path = output_dir / "search.index.jsonl"
    text = "".join(
        json.dumps(search_index_record(record), ensure_ascii=False, separators=(",", ":")) + "\n"
        for record in records
    )
...
def read_search_index(index_path: Path) -> list[dict[str, Any]]:
    ...
    try:
        ...
        raise SystemExit(f"Could not read search index {index_path}: {exc}") from exc
    return rows


def launch_gui(index_path: Path) -> None:
    """Dependency-free Tkinter navigator backed only by search.index.jsonl."""
    try:
        import tkinter as tk
        from tkinter import ttk
    except ImportError as exc:
        raise SystemExit(
            "Tkinter is unavailable. Install your distribution's python3-tk package "
            "or run with --no-gui."
        ) from exc

    rows = read_search_index(index_path)
    root = tk.Tk()
    root.title("R2 Works Navigator")
    root.geometry("1240x760")
    root.minsize(820, 520)

    query_var = tk.StringVar()
    field_var = tk.StringVar(value="Everything")
    status_var = tk.StringVar(value="Any status")
    archive_var = tk.StringVar(value="Any archive")
    count_var = tk.StringVar()
    footer_var = tk.StringVar(value=f"Canonical index: {index_path}")
    ...
    ttk.Label(toolbar, text="Field").grid(row=0, column=1, sticky="w")
    ttk.Combobox(toolbar, textvariable=field_var, state="readonly", width=16,
                 values=("Everything", "Name", "Paths", "Manifest URLs")).grid(row=1, column=1, padx=(0, 8))
    ttk.Label(toolbar, text="State").grid(row=0, column=2, sticky="w")
    ttk.Combobox(toolbar, textvariable=status_var, state="readonly", width=14,
                 values=("Any status", "complete", "incomplete", "unresolved")).grid(row=1, column=2, padx=(0, 8))
    ttk.Label(toolbar, text="Archive").grid(row=0, column=3, sticky="w")
    ttk.Combobox(toolbar, textvariable=archive_var, state="readonly", width=14,
                 values=("Any archive", "Has archive", "ZIP", "CBZ", "No archive")).grid(row=1, column=3, padx=(0, 8))
    ttk.Label(toolbar, textvariable=count_var).grid(row=1, column=4, sticky="e")
    ...
    tree = ttk.Treeview(left, columns=("state", "items", "archive"), show="tree headings", selectmode="browse")
    tree.heading("#0", text="Work")
    tree.heading("state", text="State")
    tree.heading("items", text="Items")
    tree.heading("archive", text="Archives")
    tree.column("#0", width=330, stretch=True)
    tree.column("state", width=90, anchor="center", stretch=False)
    tree.column("items", width=60, anchor="center", stretch=False)
    tree.column("archive", width=70, anchor="center", stretch=False)
    ...
    notebook.add(summary_frame, text="Details")
    notebook.add(manifest_frame, text="Manifests and URLs")
    notebook.add(raw_frame, text="Raw index row")

    def make_text(parent: object) -> tk.Text:
    ...
    details_text = make_text(summary_frame)
    manifests_text = make_text(manifest_frame)
    raw_text = make_text(raw_frame)
    footer = ttk.Label(root, textvariable=footer_var, padding=(12, 0, 12, 10))
    footer.pack(fill="x")
    ...
    def field_text(row: dict[str, Any]) -> str:
        ...
        if field == "Paths":
            values = [row.get("remote_path", "")]
            for key in ("archive_paths", "details_paths", "item_json_paths", "thumb_paths"):
                values.extend(row.get(key) or [])
            return " ".join(map(str, values)).casefold()
        if field == "Manifest URLs":
            values = []
            for manifest in row.get("manifests") or []:
                values.extend(str(manifest.get(key) or "") for key in (
                    "base_url", "url_template", "first_page_url", "last_page_url"
                ))
            return " ".join(values).casefold()
        return str(row.get("search_text") or "").casefold()

    def matches(row: dict[str, Any]) -> bool:
        terms = [part.casefold() for part in query_var.get().split() if part.strip()]
        haystack = field_text(row)
        if any(term not in haystack for term in terms):
            return False
        selected_status = status_var.get()
        if selected_status != "Any status" and row.get("status") != selected_status:
            return False
        archive_filter = archive_var.get()
        archives = int(row.get("archive_count") or 0)
        if archive_filter == "Has archive" and archives == 0:
            return False
        if archive_filter == "ZIP" and int(row.get("zip_count") or 0) == 0:
            return False
        if archive_filter == "CBZ" and int(row.get("cbz_count") or 0) == 0:
            return False
        if archive_filter == "No archive" and archives != 0:
            return False
        return True

    def show_row(row: dict[str, Any]) -> None:
        summary = [
            str(row.get("name") or ""),
            "=" * len(str(row.get("name") or "")),
            f"State: {row.get('status', 'unknown')}",
            f"Remote directory: {row.get('remote_path', '')}",
            "",
            ...
            f"ZIP files: {row.get('zip_count', 0)}",
            f"CBZ files: {row.get('cbz_count', 0)}",
        ]
        for label, key in (
            ("details.json paths", "details_paths"),
            ("thumb paths", "thumb_paths"),
            ("archive paths", "archive_paths"),
            ("item.json paths", "item_json_paths"),
        ):
            values = row.get(key) or []
            if values:
                summary.extend(["", label + ":", *[f"  {value}" for value in values]])
        if row.get("possible_duplicate"):
            summary.extend(["", f"Possible duplicate group: {row['possible_duplicate']}"])
        set_text(details_text, "\n".join(summary))

        manifest_lines = []
        manifests = row.get("manifests") or []
        if not manifests:
            manifest_lines.append("No item.json manifests were discovered for this work.")
        for number, manifest in enumerate(manifests, start=1):
            manifest_lines.extend([f"Manifest {number}", "-" * 40, f"item.json: {manifest.get('item_path', '')}"])
            if manifest.get("valid"):
                manifest_lines.extend([
                    f"Base URL: {manifest.get('base_url', '')}",
                    f"Pages predicted: {manifest.get('pages', 0)}",
                    f"Padding: {manifest.get('padding', '')}",
                    f"Extension: {manifest.get('extension', '')}",
                    f"URL template: {manifest.get('url_template', '')}",
                    f"First page: {manifest.get('first_page_url', '')}",
                    f"Last page: {manifest.get('last_page_url', '')}",
                ])
            else:
                manifest_lines.append(f"Unresolved: {manifest.get('error', 'unknown error')}")
            manifest_lines.append("")
        set_text(manifests_text, "\n".join(manifest_lines))
    ...
    def refresh(*_args: object) -> None:
        ...
        if selected:
            ...
            prior_name = tree.item(selected[0], "text")
        visible[:] = [row for row in rows if matches(row)]
        visible.sort(key=lambda row: natural_key(str(row.get("name") or "")))
        tree.delete(*tree.get_children())
        selected_id = None
        for index, row in enumerate(visible):
            item_id = tree.insert("", "end", text=row.get("name", ""), values=(
                row.get("status", "unknown"), row.get("item_json_count", 0), row.get("archive_count", 0)
            ))
            if prior_name and row.get("name") == prior_name:
                selected_id = item_id
        count_var.set(f"{len(visible)} / {len(rows)} works")
        if visible:
            selected_id = selected_id or tree.get_children()[0]
            tree.selection_set(selected_id)
            tree.focus(selected_id)
            tree.see(selected_id)
            show_row(visible[tree.index(selected_id)])
        else:
            set_text(details_text, "No matching works.")
            set_text(manifests_text, "No matching works.")
            set_text(raw_text, "")

    def selected(_event: object = None) -> None:
        selection = tree.selection()
        if selection:
            index = tree.index(selection[0])
            if 0 <= index < len(visible):
                show_row(visible[index])

    def focus_results(_event: object = None) -> str:
    ...
    for variable in (query_var, field_var, status_var, archive_var):
        variable.trace_add("write", refresh)
    tree.bind("<<TreeviewSelect>>", selected)
    search.bind("<Down>", focus_results)
    search.bind("<Return>", focus_results)
    search.focus_set()
    refresh()
    root.mainloop()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--no-gui", action="store_true", help="Do not launch the searchable GUI after rebuilding the index.")
...
def main() -> None:
    ...
    index_path = write_search_index(records, args.output_dir)
    print(f"Search index written: {index_path}")
    ...
    if args.interactive:
        ...
        launch_gui(index_path)
    elif not performed_action:
        print_work_table(records)
