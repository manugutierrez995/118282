...
def search_index_record(record: WorkRecord) -> dict[str, Any]:
    return {
        ...
        "item_manifests": record.item_manifests,
...
def write_search_index(records: list[WorkRecord], output_dir: Path) -> Path:
    ...
    text = "".join(
        json.dumps(search_index_record(record), ensure_ascii=False) + "\n"
        for record in records
    )
...
def launch_gui(records: list[WorkRecord], index_path: Path) -> None:
    try:
    ...
    root = tk.Tk()
    root.title("R2 Work Auditor")
    root.geometry("1080x700")
    root.minsize(760, 480)

    query_var = tk.StringVar()
    count_var = tk.StringVar()
    status_var = tk.StringVar(value=f"Index: {index_path}")

    top = ttk.Frame(root, padding=12)
    top.pack(fill="x")
    ttk.Label(top, text="Search works:").pack(side="left")
    ...
    details = tk.Text(right, wrap="word", state="disabled")
    details_scroll = ttk.Scrollbar(right, orient="vertical", command=details.yview)
    ...
    visible: list[WorkRecord] = []

    def show_record(record: WorkRecord) -> None:
        lines = [
            record.name,
            "=" * len(record.name),
            f"Remote: {record.remote_path}",
            "",
            f"details.json: {'yes' if record.has_details_json else 'no'}",
            f"thumb.webp: {'yes' if record.has_thumb_webp else 'no'}",
            f"item.json files: {record.item_json_count}",
            f"ZIP files: {record.zip_count}",
            f"CBZ files: {record.cbz_count}",
        ]
        if record.archive_paths:
            lines += ["", "Archives:"] + [f"  {p}" for p in record.archive_paths]
        if record.item_manifests:
            lines += ["", "Predicted page URLs:"]
            for manifest in record.item_manifests:
                if manifest.get("valid"):
                    lines += [
                        f"  {manifest['url_template']}",
                        f"    pages: {manifest['pages']}",
                        f"    first: {manifest['first_page_url']}",
                        f"    last:  {manifest['last_page_url']}",
                    ]
                else:
                    lines.append(
                        f"  {manifest.get('item_path')}: unresolved ({manifest.get('error', 'unknown')})"
                    )
        if record.suspicious_duplicate_group:
            lines += ["", f"Possible duplicate group: {record.suspicious_duplicate_group}"]
    ...
    def refresh(*_args: object) -> None:
        ...
        visible[:] = [r for r in records if needle in r.name.casefold()]
        listbox.delete(0, "end")
        for record in visible:
            listbox.insert("end", record.name)
        count_var.set(f"{len(visible)} / {len(records)}")
        if visible:
            listbox.selection_set(0)
            show_record(visible[0])
        else:
            details.configure(state="normal")
            details.delete("1.0", "end")
            details.insert("1.0", "No matching works.")
            details.configure(state="disabled")

    def selected(_event: object = None) -> None:
        selection = listbox.curselection()
        if selection:
            show_record(visible[selection[0]])

    query_var.trace_add("write", refresh)
    listbox.bind("<<ListboxSelect>>", selected)
    search.bind("<Down>", lambda _e: (listbox.focus_set(), "break")[1])
    search.focus_set()
    refresh()
    root.mainloop()
