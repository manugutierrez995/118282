...
def default_tags(name: str = "") -> dict[str, Any]:
    # Public and private are defaults, not the only permitted top-level blocks.
    out: dict[str, Any] = {"schema_version": 1, "public": {}, "private": {}}
    if name:
        out["name"] = name
    return out
...
def normalize_tags_file(value: Any) -> dict[str, Any]:
    """Preserve arbitrary top-level tag blocks while guaranteeing defaults."""
    if not isinstance(value, dict):
        value = {}
    ...
    for key, child in value.items():
        ...
        if name:
            out[name] = normalize_tag_tree(child)
    out.setdefault("public", {})
    out.setdefault("private", {})
    return out


def tag_blocks(tags: dict[str, Any]) -> dict[str, dict[str, Any]]:
    normalized = normalize_tags_file(tags)
    return {key: value for key, value in normalized.items() if key not in {"schema_version", "name"}}
...
def build_search_text(row: dict[str, Any]) -> str:
    ...
    for block_name, tree in tag_blocks(tags).items():
        values.append(block_name)
        values.extend(flatten_tag_paths(tree, (block_name,)))
...
def call_tags(remote: str, row: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        row["tags"] = default_tags(row["name"])
...
def launch_gui(remote: str, output_dir: Path, catalog: dict[str, dict[str, Any]]) -> None:
    ...
    tag_frame = ttk.Frame(tags_tab, padding=6)
    tag_frame.pack(fill="both", expand=True)
    tag_tree = ttk.Treeview(tag_frame, show="tree", selectmode="browse")
    tag_scroll = ttk.Scrollbar(tag_frame, orient="vertical", command=tag_tree.yview)
    tag_tree.configure(yscrollcommand=tag_scroll.set)
    tag_tree.pack(side="left", fill="both", expand=True)
    tag_scroll.pack(side="right", fill="y")
    tag_buttons = ttk.Frame(tags_tab, padding=(6, 0, 6, 6))
    tag_buttons.pack(fill="x")
    ttk.Label(
        tag_buttons,
        text="Select a block/tag to add beneath it. Clear selection to add a new top-level block.",
    ).pack(side="left", padx=(0, 10))

    ttk.Label(right, text="Thumbnail", font=("TkDefaultFont", 11, "bold")).pack(anchor="w")
    thumb_label = ttk.Label(right, text="Select a work to display its cached thumbnail.", anchor="center", justify="center")
    ...
    ttk.Label(right, text="Paths and clickable URLs", font=("TkDefaultFont", 11, "bold")).pack(anchor="w")
    ttk.Label(
        right,
        text="Program locations appear first; complete usable URLs appear below.",
        wraplength=330,
        justify="left",
    ).pack(anchor="w", pady=(0, 5))
    urls_text = tk.Text(right, wrap="word", height=15, padx=8, pady=8, cursor="arrow")
    urls_scroll = ttk.Scrollbar(right, orient="vertical", command=urls_text.yview)
    urls_text.configure(yscrollcommand=urls_scroll.set)
    urls_text.pack(side="left", fill="both", expand=True, pady=(0, 4))
    urls_scroll.pack(side="right", fill="y", pady=(0, 4))
    urls_text.tag_configure("heading", font=("TkDefaultFont", 10, "bold"), spacing1=5, spacing3=3)
    urls_text.tag_configure("internal", foreground="#666666")
    urls_text.tag_configure("url", underline=True)
    urls_text.configure(state="disabled")
    ...
    def collect_http_urls(value: Any) -> list[str]:
        found: list[str] = []
        seen: set[str] = set()
    ...
    def render_paths_and_urls(row: dict[str, Any]) -> None:
        urls_text.configure(state="normal")
        urls_text.delete("1.0", "end")

        def line(text: str = "", tag: str | None = None) -> None:
            start = urls_text.index("end-1c")
            urls_text.insert("end", text + "\n")
            if tag:
                end = urls_text.index("end-1c")
                urls_text.tag_add(tag, start, end)

        line("PROGRAM USES", "heading")
        remote_path = str(row.get("remote_path") or row.get("name") or "")
        line(f"Appended work path: {remote_path}", "internal")
        line(f"Rclone work URI: {remote_join(remote, remote_path)}", "internal")

        for manifest in row.get("manifests") or []:
            if not isinstance(manifest, dict):
                continue
            item_path = str(manifest.get("item_path") or "")
            base_url = str(manifest.get("base_url") or "")
            if item_path:
                line(f"Appended item path: {item_path}", "internal")
                line(f"Rclone item URI: {remote_join(remote, item_path)}", "internal")
            if base_url:
                line(f"Base URL used: {base_url}", "internal")
            if manifest.get("url_template"):
                line(f"Generated template: {manifest['url_template']}", "internal")

        line()
        line("FINAL CLICKABLE URLS", "heading")
        candidates = collect_http_urls({
            "manifests": row.get("manifests") or [],
            "resources": row.get("resources") or {},
        })
        if not candidates:
            line("No complete HTTP/HTTPS URLs have been loaded for this work yet.", "internal")
        else:
            for index, url in enumerate(candidates):
                tag = f"click_url_{index}"
                start = urls_text.index("end-1c")
                urls_text.insert("end", url + "\n")
                end = urls_text.index("end-1c")
                urls_text.tag_add("url", start, end)
                urls_text.tag_add(tag, start, end)
                urls_text.tag_bind(tag, "<Enter>", lambda _e: urls_text.configure(cursor="hand2"))
                urls_text.tag_bind(tag, "<Leave>", lambda _e: urls_text.configure(cursor="arrow"))
                urls_text.tag_bind(tag, "<Button-1>", lambda _e, target=url: webbrowser.open(target))
    ...
    def display_row(row: dict[str, Any]) -> None:
        ...
        fill_tag_tree(tag_tree, tag_blocks(tags))
    ...
    def refresh_list(*_args: Any) -> None:
        ...
        for row in visible:
            webp, png = cache_paths(row)
            cached = webp.exists() or png.exists()
            item = works.insert("", "end", text=row["name"], values=(row.get("status", ""), row.get("item_json_count", 0), "yes" if cached else ""))
            if row["name"] == prior:
                select_id = item
    ...
    def select_all_works(_event: Any = None) -> str:
        items = works.get_children()
        if items:
            works.selection_set(items)
            works.focus(items[0])
            works.see(items[0])
            on_select()
            status_var.set(f"Selected all {len(items)} visible work(s).")
        return "break"

    # Ctrl+A selects every currently visible work whenever the work list has focus.
    works.bind("<Control-a>", select_all_works)
    works.bind("<Control-A>", select_all_works)
    ...
    def do_selected(label: str, function: Callable[[str, dict[str, Any]], dict[str, Any]]) -> None:
        ...
        for number, row in enumerate(targets, 1):
            status_var.set(f"{label} {number}/{len(targets)}: {row['name']}"); root.update_idletasks()
            try:
        ...
        status_var.set(f"{label} finished for {len(targets)} selected work(s). Failures: {failures}")
        refresh_list(); schedule_search_rebuild()

    def call_thumb(row: dict[str, Any]) -> dict[str, Any]:
        webp, png = cache_paths(row)

        # A locally cached thumbnail is already complete; do not call R2 again.
        if webp.exists() or png.exists():
            cached = png if png.exists() else webp
            row.setdefault("resources", {})["thumbnail_cache"] = str(cached)
            row["thumb_called_at"] = row.get("thumb_called_at") or utc_iso()
            return row
    ...
    def ensure_tags_file(_remote: str, row: dict[str, Any]) -> dict[str, Any]:
        if not row.get("scanned_at"):
            row = structural_scan(remote, row["name"], row)
        current = normalize_tags_file(row.get("tags"))
        current["name"] = row["name"]
        row["tags"] = current
        return save_tags(remote, row)
    ...
    ttk.Button(actionbar, text="Select all works", command=select_all_works).pack(side="left", padx=(0, 6))
    selected_menu = ttk.Menubutton(actionbar, text="Selected works…")
    menu = tk.Menu(selected_menu, tearoff=False)
    menu.add_command(label="Call thumbnails", command=lambda: do_selected("Call thumbnails", lambda _r, x: call_thumb(x)))
    menu.add_command(label="Call details.json", command=lambda: do_selected("Call details.json", call_details))
    menu.add_command(label="Call tags.json", command=lambda: do_selected("Call tags.json", call_tags))
    menu.add_command(label="Create/ensure tags.json", command=lambda: do_selected("Create/ensure tags.json", ensure_tags_file))
    menu.add_command(label="Call item.json", command=lambda: do_selected("Call item.json", call_items))
    menu.add_command(label="Call everything", command=lambda: do_selected("Call everything", call_all_resources))
    menu.add_separator()
    menu.add_command(label="Download details.json", command=lambda: download_json_for_rows("details", selected_rows(), "Download selected details.json"))
    menu.add_command(label="Download tags.json", command=lambda: download_json_for_rows("tags", selected_rows(), "Download selected tags.json"))
    menu.add_command(label="Download item.json", command=lambda: download_json_for_rows("items", selected_rows(), "Download selected item.json"))
    menu.add_command(label="Download all JSON", command=lambda: download_json_for_rows("all-json", selected_rows(), "Download selected JSON"))
    selected_menu["menu"] = menu
    selected_menu.pack(side="left")

    def selected_tag_parts() -> list[str]:
        selection = tag_tree.selection()
        return tree_path(tag_tree, selection[0]) if selection else []

    def add_tag_node() -> None:
        row = selected_required()
        if row is None:
            return
        parent_parts = selected_tag_parts()
        prompt = "New child block/tag name:" if parent_parts else "New top-level block name:"
        value = simpledialog.askstring("Add tag block", prompt, parent=root)
        if not value or not value.strip():
            return
        name = value.strip()
        tags = normalize_tags_file(row.get("tags"))
        blocks = tag_blocks(tags)
        node: dict[str, Any] = blocks
        ...
        tags = {"schema_version": 1, "name": row["name"], **blocks}
        row["tags"] = tags
        try:
    ...
    def add_comma_tags() -> None:
        ...
        if not parent_parts:
            messagebox.showinfo(
                "Select a block",
                "Select the block that should receive these tags first. "
                "For example, select public, private, genre, or another block.",
            )
            return
        ...
        ttk.Label(
            body,
            text=f"Add tags beneath: {' > '.join(parent_parts)}",
        ).pack(anchor="w")
        ttk.Label(
            body,
            text="Separate tags with commas, then press Enter.",
        ).pack(anchor="w", pady=(2, 8))

        value_var = tk.StringVar()
        entry = ttk.Entry(body, textvariable=value_var, width=64)
        entry.pack(fill="x")
        button_row = ttk.Frame(body)
        button_row.pack(fill="x", pady=(10, 0))

        def submit(_event: Any = None) -> None:
            values = [part.strip() for part in value_var.get().split(",") if part.strip()]
            if not values:
                return
            tags = normalize_tags_file(row.get("tags"))
            blocks = tag_blocks(tags)
            ...
            row["tags"] = tags
            try:
                persist(save_tags(remote, row), f"Added {len(values)} tag(s)")
            except Exception as exc:
                messagebox.showerror("Save tags", str(exc), parent=dialog)
                return
            dialog.destroy()

        ttk.Button(button_row, text="Cancel", command=dialog.destroy).pack(side="right")
        ttk.Button(button_row, text="Add tags", command=submit).pack(side="right", padx=(0, 6))
        entry.bind("<Return>", submit)
        entry.bind("<KP_Enter>", submit)
        dialog.bind("<Escape>", lambda _event: dialog.destroy())
        entry.focus_set()

    def remove_tag_node() -> None:
        row = selected_required()
        parts = selected_tag_parts()
        if row is None or not parts:
            return
        if parts[0] in {"public", "private"} and len(parts) == 1:
            messagebox.showinfo("Default block", "The default public and private blocks cannot be removed.")
            return
        tags = normalize_tags_file(row.get("tags"))
        blocks = tag_blocks(tags)
        delete_tag_path(blocks, parts)
        tags = {"schema_version": 1, "name": row["name"], **blocks}
        row["tags"] = tags
        try:
            persist(save_tags(remote, row), "Saved tags")
    ...
    def clear_tag_selection() -> None:
        tag_tree.selection_remove(tag_tree.selection())

    ttk.Button(tag_buttons, text="+ Add block", command=add_tag_node).pack(side="right", padx=(5, 0))
    ttk.Button(tag_buttons, text="Add tags…", command=add_comma_tags).pack(side="right", padx=(5, 0))
    ttk.Button(tag_buttons, text="Remove", command=remove_tag_node).pack(side="right", padx=(5, 0))
    ttk.Button(tag_buttons, text="Top level", command=clear_tag_selection).pack(side="right", padx=(5, 0))
