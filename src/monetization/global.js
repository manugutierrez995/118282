...
export function initializeGlobalMonetization() {
  ...
  const cleanups = [], root = document.createElement("div"); root.id = "doku-interstitial-root"; root.className = "doku-global-formats"; document.body.append(root);
  const interstitial = innerWidth < 600 ? "mobile_full_page_interstitial" : "desktop_full_page_interstitial";
  for (const id of [interstitial, "global_popunder"]) { const mount=document.createElement("div"); mount.hidden=true; root.append(mount); cleanups.push(renderAdPlacement(mount,id)); }
